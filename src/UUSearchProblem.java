...
public abstract class UUSearchProblem {
	...
	private List<UUSearchNode> backchain(UUSearchNode node,
		...
		UUSearchNode prevNode = visited.get(node);
		result.add(node);
		while (prevNode != null){
			node = prevNode;
			result.add(node);
			prevNode = visited.get(node);
		}
		
		return reverseList(result);
	}
	
	private List<UUSearchNode> reverseList(List<UUSearchNode> result){
		// reverse list of UUSearchNodes
		int i = 0;
		int j = result.size() - 1;
		while (i <= j){
			UUSearchNode temp = result.get(i);
